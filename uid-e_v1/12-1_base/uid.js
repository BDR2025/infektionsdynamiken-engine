 ...
 * Version:  v1.0.3
 * Changelog:
 *   - v1.0.3 E0 in model:update aufgenommen; im Bulk-Pfad zusätzliche
 *            Guard-Constraint (E0 ≤ N − I0, nie negativ). Einzelkey-Änderungen
 *            weiter über applyCouplings() abgesichert.
...
export function createUID(cfg = {}) {
  ...
  function recalc() {
    ...
    emit('uid:e:model:update', {
      N: params.N, I0: params.I0, E0: params.E0,               // ← E0 ergänzt
  ...
  on('uid:e:params:change', (payload) => {
    ...
    if (payload.bulk && typeof payload.bulk === 'object') {
      // Bulk: zunächst normalisieren …
      params = normalizeParams({ ...params, ...payload.bulk }, catalog);

      // … dann Guard-Constraint anwenden (Bulk ruft applyCouplings nicht auf):
      // E0 darf nicht größer als N − I0 und nie negativ sein.
